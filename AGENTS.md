...
- **The About photo is row-height driven, not square**: `.about-grid` uses `align-items: stretch` and `.about-photo` has `height: 100%` with `aspect-ratio` removed, so the frame matches the height of the text column (~830px at desktop). The 800×1200 source is a portrait, so `object-fit: cover` on a 320×830 box shows the **full height** and only the central ~58% of the width — no vertical crop, which the old `aspect-ratio: 1 / 1` square was doing (it threw away a third of the picture's height). The `≤900px` block **must** restore `height: auto; aspect-ratio: 1 / 1`: in a single-column grid there is no row to resolve `height: 100%` against and the image collapses to nothing.
